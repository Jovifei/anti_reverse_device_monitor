...
async function main() {
  ...
  if (!configured && dryRun) {
    console.log(JSON.stringify({ status: 'dry-run', sourceConfigured: false, recordsRead: 0, message: 'No approved source is configured; validated the offline sync command without opening SQLite.' }, null, 2))
    return
  }
  if (!process.env.APP_DATABASE_URL) throw new Error('APP_DATABASE_URL is required for a non-dry-run source synchronization.')
  const from = value('--from'); const adapter = configured ? createConfiguredSourceAdapter() : new MockSourceAdapter(); const result = await new SourceSyncService(adapter).sync({ dryRun, from: from ? new Date(from) : undefined, sn: value('--sn') }); console.log(JSON.stringify(result, null, 2)); await prisma.$disconnect(); if (result.status === 'failed') process.exitCode = 1
}
