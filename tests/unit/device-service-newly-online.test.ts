...
function dashboardRecord(deviceSn: string, options: { platformOnline: boolean; lastReportedAt: Date | null; createdAt?: Date }) {
  return {
    ...
    createdAt: options.createdAt ?? daysAgo(8),
...
const mockRegistry: DeviceRegistry = {
  ...
  devices: [
    ...
    { device_id: 'd_reappeared', sn: 'SN_REAPPEARED', online: false },
...
const mockDashboardRecords = [
  ...
  dashboardRecord('SN_RECENT', { platformOnline: true, lastReportedAt: daysAgo(0.1), createdAt: daysAgo(2) }),
  dashboardRecord('SN_BOUNDARY', { platformOnline: false, lastReportedAt: daysAgo(1), createdAt: daysAgo(7) }),
  dashboardRecord('SN_GHOST_CONTINUOUS', { platformOnline: true, lastReportedAt: daysAgo(0.1) }),
  dashboardRecord('SN_REAPPEARED', { platformOnline: false, lastReportedAt: daysAgo(0.1), createdAt: daysAgo(8) }),
...
describe('DeviceService.listDevices 近7日新上线', () => {
  ...
  it('只统计最近 7 日首次进入本地监控的活跃设备', async () => {
    const result = await service.listDevices({})
    expect(result.summary.activeTotal).toBe(5)
    expect(result.summary.newlyOnlineCount).toBe(2)
  })

  it('已有设备重新出现上报时不误判为新上线', async () => {
    ...
    expect(result.items.some((item) => item.deviceSn === 'SN_REAPPEARED')).toBe(false)
  })

  it('最近 7 日边界包含在新上线窗口内', async () => {
    const result = await service.listDevices({ status: 'newly-online' })
