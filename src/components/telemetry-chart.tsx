...
export function TelemetryChart({ title, series, height = 430, initialSelectedKeys, advancedKeys = [] }: Props) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      tooltip: { trigger: 'axis', backgroundColor: '#17233a', borderWidth: 0, textStyle: { color: '#fff' }, padding: [9, 12] },
      legend: { top: 7, type: 'scroll', textStyle: { color: '#667085' } },
      xAxis: { type: 'time', boundaryGap: false, axisLine: { lineStyle: { color: '#cdd7e5' } }, axisLabel: { color: '#7a8799' } },
      yAxis: { type: 'value', axisLabel: { color: '#7a8799' }, splitLine: { lineStyle: { color: '#e8edf4' } } },
      ...
      series: visible.flatMap((item) => {
        const chartPoints = item.dailyReset
          ? item.points.flatMap((point, index) => index > 0 && new Date(point[0]).getDate() !== new Date(item.points[index - 1][0]).getDate() && point[1] < item.points[index - 1][1] ? [[point[0], null] as [string, null], point] : [point])
          : item.points
        const line = {
          name: item.label,
          type: 'line' as const,
        ...
        const negative = item.markNegative && negativePoints.length > 0
          ? [{ name: `${item.label} 负值点`, type: 'scatter' as const, data: negativePoints, symbolSize: 7, itemStyle: { color: '#c92828' }, tooltip: { show: false }, silent: true }]
          : []
  ...
  const renderToggle = (item: ClientChartSeries) => <label key={item.key}><input type="checkbox" checked={selected.has(item.key)} onChange={() => toggle(item.key)} disabled={item.points.length === 0} /><i style={{ backgroundColor: item.color }} />{item.label}</label>
