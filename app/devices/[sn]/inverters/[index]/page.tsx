...
import { INVERTER_KPI_ALIASES, displayEnergyKwh, displayInverterPhaseLabel, displayPowerLimit, displaySwitch, displayValue, findLatestMetric, formatDuration, formatTime, getInverterStatus, getInverterWorkStatus, isGenerating, numericValue } from '@/src/domain/monitoring'
...
export default async function InverterPage({ params }: { params: Promise<{ sn: string; index: string }> }) {
  ...
  return <main>
    <header className="page-header"><div><p className="eyebrow">CT {summary.deviceSn}</p><h1>微型逆变器 {summary.inverterIndex}：{displayInverterPhaseLabel(summary.phaseNum)} · {summary.inverterSn ?? EMPTY}</h1><p className="muted">软件版本 {summary.softwareVersion ?? EMPTY} · Sub1G 版本 {summary.sub1gVersion ?? EMPTY}</p></div><DeviceSnSearch initialSn={summary.deviceSn} /></header>
    <section className="inverter-hero"><div className="panel-heading"><div><h2>当前运行状态</h2><p className="muted">在线、工作与发电状态独立呈现。</p></div><span className={`badge ${status.variant}`}>{status.label}</span></div><div className="status-row"><div><strong>工作状态：</strong>{getInverterWorkStatus(workRaw)}</div><div><strong>当前是否发电：</strong>{status.variant === 'online' ? (isGenerating(onlineRaw, workRaw, power) ? '正在发电' : '否') : EMPTY}</div><div><strong>当前状态持续：</strong>{formatDuration(currentStateMinutes)}</div></div></section>
    <section className="detail-grid"><MetricCard label="当前发电总功率" value={displayValue(powerRow, 'W')} series={charts.power} /><MetricCard label="PV1 功率" value={displayValue(findLatestMetric(rows, ['pv1_power', 'pv1power']), 'W')} series={charts.power.filter((item) => item.key === 'pv1')} /><MetricCard label="PV2 功率" value={displayValue(findLatestMetric(rows, ['pv2_power', 'pv2power']), 'W')} series={charts.power.filter((item) => item.key === 'pv2')} /><MetricCard label="今日发电量" value={displayEnergyKwh(findLatestMetric(rows, INVERTER_KPI_ALIASES.todayEnergy))} /><MetricCard label="累计发电量" value={displayEnergyKwh(findLatestMetric(rows, INVERTER_KPI_ALIASES.totalEnergy))} /><MetricCard label="今日发电时长" value={displayValue(findLatestMetric(rows, INVERTER_KPI_ALIASES.todayDuration), 'h')} /></section>
    <section className="panel"><h2>通信与接入配置</h2><div className="detail-grid"><MetricCard label="最近丢包率" value={displayValue(findLatestMetric(rows, INVERTER_KPI_ALIASES.packetLoss), '%')} series={charts.packetLoss} /><MetricCard label="所在相" value={displayInverterPhaseLabel(summary.phaseNum)} /><MetricCard label="接入点" value={resolveStatusLabel('connection_point', summary.connectionPoint) ?? EMPTY} /><MetricCard label="防逆流开关" value={displaySwitch(findLatestMetric(rows, INVERTER_KPI_ALIASES.antiReverse))} /><MetricCard label="发电开关" value={displaySwitch(findLatestMetric(rows, INVERTER_KPI_ALIASES.generationEnabled))} /><MetricCard label="功率限制" value={displayPowerLimit(findLatestMetric(rows, INVERTER_KPI_ALIASES.powerLimit))} /></div></section>
