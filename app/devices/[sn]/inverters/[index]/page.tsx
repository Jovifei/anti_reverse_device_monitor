...
const EMPTY = '—'
function MetricCard({ label, value }: { label: string; value: string }) { return <div className="metric-card"><div className="label">{label}</div><div className="value">{value}</div></div> }
...
export default async function InverterPage({ params }: { params: Promise<{ sn: string; index: string }> }) {
  ...
  const status = summary.paired === false && onlineRaw === null ? getInverterStatus(0) : getInverterStatus(onlineRaw)
  const latestFault = summary.faults.find((fault) => fault.faults.length > 0)
  ...
  return <main>
    ...
    <section className="two-column"><div className="panel"><h2>在线/离线状态与持续时长</h2><p>当前状态持续：{formatDuration(summary.connectivity.isOnline ? null : summary.connectivity.currentOfflineMinutes)}</p>{summary.connectivity.transitions.length ? <ul className="record-list">{summary.connectivity.transitions.map((item) => <li key={`${item.at}-${item.state}`}>{formatTime(item.at)}：{item.value === null ? (item.state === 'online' ? '上线' : '离线') : (resolveStatusLabel('inverter_online_state', item.value) ?? EMPTY)}</li>)}</ul> : <p className="muted">没有 online_state 记录。</p>}</div><div className="panel"><h2>离线区间</h2>{summary.connectivity.offlineWindows.length ? <ul className="record-list">{summary.connectivity.offlineWindows.map((item) => <li key={`${item.startAt}-${item.endAt}`}>{formatTime(item.startAt)} 至 {formatTime(item.endAt)} · {formatDuration(item.durationMinutes)}</li>)}</ul> : <p className="muted">当前窗口没有离线区间。</p>}</div></section>
    <section className="panel"><h2>最新故障</h2>{latestFault ? <><p>原始故障码：{latestFault.value === null ? EMPTY : `0x${latestFault.value.toString(16).toUpperCase().padStart(8, '0')}`}</p><div>{latestFault.faults.map((fault) => <span key={fault.name} className="fault-name">{fault.name}</span>)}</div></> : <p className="muted">{EMPTY}</p>}<h3>故障码变化记录</h3>{summary.faultChanges.length ? <ul className="record-list">{summary.faultChanges.map((event) => <li key={`${event.at}-${event.eventType}-${event.fromMask}-${event.toMask}`}>{formatTime(event.at)}：{event.eventType} · {event.fromHex} {'→'} {event.toHex}<br />{event.toFaults.length ? event.toFaults.join('、') : '故障已恢复'}</li>)}</ul> : <p className="muted">最近 7 天没有故障变化。</p>}</section>
    <Link href={`/devices/${encodeURIComponent(summary.deviceSn)}`}>返回 CT 设备页</Link>
  </main>
}
