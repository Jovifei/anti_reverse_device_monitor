...
export default async function InverterPage({ params }: { params: Promise<{ sn: string; index: string }> }) {
  ...
  return <main>
    ...
    <section className="panel"><div className="panel-heading"><h2>当前运行状态</h2><span className={`badge ${status.variant}`}>{status.label}</span></div><div className="status-row"><div><strong>工作状态：</strong>{getInverterWorkStatus(workRaw)}</div><div><strong>当前是否发电：</strong>{isGenerating(numericValue(findLatestMetric(rows, INVERTER_KPI_ALIASES.generating)), workRaw) ? '正在发电' : '—'}</div></div></section>
