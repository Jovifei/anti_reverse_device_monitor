...
import { INVERTER_KPI_ALIASES, displayEnergyKwh, displayPowerLimit, displaySwitch, displayValue, findLatestMetric, formatDuration, formatTime, getInverterStatus, getInverterWorkStatus, isGenerating, numericValue } from '@/src/domain/monitoring'
...
export default async function InverterPage({ params }: { params: Promise<{ sn: string; index: string }> }) {
  ...
  return <main>
    <header className="page-header"><div><p className="eyebrow">CT {summary.deviceSn}</p><h1>微型逆变器 {summary.inverterIndex}：{summary.inverterSn ?? EMPTY}</h1><p className="muted">软件版本 {summary.softwareVersion ?? EMPTY} · Sub1G 版本 {summary.sub1gVersion ?? EMPTY}</p></div><DeviceSnSearch initialSn={summary.deviceSn} /></header>
