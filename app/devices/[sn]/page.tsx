...
export default async function DeviceDetailPage({ params }: { params: Promise<{ sn: string }> }) {
  ...
  return <main>
    ...
    <section className="panel"><div className="panel-heading"><div><h2>CT 本体运行状态</h2><p className="muted">平台连通性与关键运行枚举</p></div><span className={`badge ${history.platform.isOnline ? 'online' : 'offline'}`}>{history.platform.isOnline ? '平台在线' : '平台离线'}</span></div><ul className="status-list"><li>当前离线持续时长<br /><strong>{history.platform.isOnline ? EMPTY : formatDuration(currentOffline)}</strong></li><li>运行状态 state<br /><strong>{resolveStatusLabel('ct_state', numericValue(findLatestMetric(latest, CT_KPI_ALIASES.state))) ?? EMPTY}</strong></li><li>限流状态<br /><strong>{resolveStatusLabel('limit_state', numericValue(findLatestMetric(latest, CT_KPI_ALIASES.limitState))) ?? EMPTY}</strong></li><li>Sub1G 状态<br /><strong>{resolveStatusLabel('sub1g_state', numericValue(findLatestMetric(latest, CT_KPI_ALIASES.sub1gState))) ?? EMPTY}</strong></li><li>工作模式<br /><strong>{resolveStatusLabel('work_mode', numericValue(findLatestMetric(latest, CT_KPI_ALIASES.workMode))) ?? EMPTY}</strong></li><li>最后状态变化<br /><strong>{formatTime(history.platform.transitions.at(-1)?.at)}</strong></li></ul></section>

    <section className="panel"><div className="panel-heading"><div><h2>防逆流安全状态</h2><p className="muted">任一相 CT 有功功率低于 0 W 时触发严重告警。</p></div><span className={`badge ${phaseRows.some((item) => (numericValue(item.row) ?? 0) < 0) ? 'danger' : 'online'}`}>{phaseRows.some((item) => (numericValue(item.row) ?? 0) < 0) ? '严重告警：正在反送电网' : '当前无逆流'}</span></div><div className="phase-grid">{phaseRows.map(({ phase, row }) => { const reverse = (numericValue(row) ?? 0) < 0; return <div key={phase} className={`phase-card ${reverse ? 'danger' : ''}`}><span className="phase-label">{phase} 相 CT 有功功率</span><strong className="phase-value">{displayValue(row, 'W')}</strong><span className="phase-hint">{reverse ? '功率正在反送电网' : '当前相功率正常'}</span></div> })}</div><h3>最近 7 天逆流告警记录</h3>{alarms.alerts.length ? <ul className="record-list">{alarms.alerts.map((alert) => <li key={`${alert.phase}-${alert.startedAt}`}><strong>{alert.phase} 相严重告警</strong> · 开始 {formatTime(alert.startedAt)} · 恢复 {alert.endedAt ? formatTime(alert.endedAt) : '持续中'} · 持续 {formatDuration(alert.durationMinutes)} · 最低功率 <span className="danger-value">{alert.minimumPower} W</span> · 样本 {alert.sampleCount}</li>)}</ul> : <p className="muted">最近 7 天没有检测到三相 CT 负功率。</p>}</section>
    ...
    <section className="panel"><div className="inverter-section-title"><div><h2>微型逆变器 1～8</h2><p className="muted">独立显示每台微逆的通讯、发电与故障状态。</p></div><span className="muted">固定 8 通道</span></div><div className="inverter-grid">{Array.from({ length: 8 }, (_, offset) => {
      ...
      const isUnpaired = binding?.paired === false
      const status = isUnpaired ? getInverterStatus(0) : getInverterStatus(onlineRaw)
